...
uint16_t fill_tcp_data_len(uint8_t *buf,uint16_t pos, const char *s, uint16_t len)
{
       ...
       // usart_SendString (UART4,"\r\n=> TCP DATA:");usart_SendString_Len(UART4, buf,pos);
...
void make_tcp_ack_with_data_noflags(uint8_t *buf,uint16_t dlen)
{
        ...
        //usart_SendString (UART4,"\r\nEthernet Frame:");usart_SendString_Len(UART4,buf,IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+dlen+ETH_HEADER_LEN);
}
...
void www_server_reply(uint8_t *buf,uint16_t dlen)
{
        ...
        // This code requires that we send only one data packet  because we keep no state information. We must therefore set
        // the fin here:1444
        buf[TCP_FLAGS_P]=TCP_FLAGS_ACK_V|TCP_FLAGS_PUSH_V|TCP_FLAGS_FIN_V;
...
void client_icmp_request(uint8_t *buf,uint8_t *destip)
{
        ...
        buf[IP_TOTLEN_L_P]=0x54;        // TUX Code has 0x54, here has 0x82
...
void client_arp_whohas(uint8_t *buf,uint8_t *ip_we_search)
{
        ...
        while(i<6){
                buf[ETH_DST_MAC +i]=0xff;
                buf[ETH_SRC_MAC +i]=macaddr[i];		//{0x54,0x55,0x58,0x10,0x00,0x25};
                i++;
        }
...
uint16_t www_client_internal_datafill_callback(uint8_t fd){
        ...
        if (fd==www_fd){
                if (browsertype==0){
                        ...
                        len=fill_tcp_data(bufptr,len,"\r\nUser-Agent: EtherShield/1.6\r\nAccept: */*\r\nConnection: close\r\n"); //connection type change (close->keep-alive)
                        ...
                        len=fill_tcp_data(bufptr,len,"\r\nContent-Type: application/x-www-form-urlencoded\r\n\r\n"); //content type change (application/x-www-form-urlencoded -> text/plain)
...
//usart_SendString(UART5, "\r\n->Entered in packetloop_icmp_tcp.");
//usart_SendString(UART5, "\r\n->Buf:");usart_SendString(UART5, buf);
