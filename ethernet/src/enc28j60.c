...
void enc28j60PacketSend(uint16_t len, uint8_t* packet)
{
	...
	enc28j60WriteWord(EWRPTL, TXSTART_INIT);						// Set the write pointer to start of transmit buffer area
	enc28j60WriteWord(ETXNDL, (TXSTART_INIT+len));					// Set the TXND pointer to correspond to the packet size given
	enc28j60WriteOp(ENC28J60_WRITE_BUF_MEM, 0, 0x00);				// write per-packet control byte (0x00 means use macon3 settings)
